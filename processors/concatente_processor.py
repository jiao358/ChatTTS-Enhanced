...
def concatenate_audio(audio_files, output_path):
    with open("filelist.txt", "w", encoding="utf-8") as f:
        for audio_file in audio_files:
            f.write(f"file {shlex.quote(audio_file)}\n")
    
    cmd = f'ffmpeg -y -loglevel error -f concat -safe 0 -i filelist.txt -c copy "{output_path}"'
    os.system(cmd)
...
def concatenate_audiofile(file_name, audio_files, enhanced_audio_files):
    ...
    out_con_audio = os.path.join(get_path('OUTPUT_DIR'), safe_file_name, '合并')
    concatenated_audio_path = os.path.join(out_con_audio, f"{safe_file_name}_合并.wav")
    ...
    if len(enhanced_audio_files) > 1:
        concatenated_enhanced_audio_path = os.path.join(out_con_audio, f"{safe_file_name}_合并_增强.wav")
